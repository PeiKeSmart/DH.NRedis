...
| **严谨** | 需求理解度必须达到 95% 以上再开始编码，不清楚必须主动提问 |
| **求证** | 遇到未知、不确定或多次尝试失败的问题，主动通过搜索工具获取最新信息，不盲目猜测 |
| **主动** | 发现问题主动修复，不回避合理优化 |
...
- 需求包含：XCode/NCube/魔方/实体生成/模型 XML/数据类库/数据库 CRUD/Controller 生成/`xcodetool`/`xcode` 命令
- 解决方案/项目中出现：`DH.NCode` 包引用
...
未满足以上条件时，**不要**引入 XCode/NCube 初始化流程，避免干扰其它仓库的常规开发。
...
1. **需求分类**：功能/修复/性能/重构/文档。**在开始编码前，必须确保对需求的理解度达到 95% 以上。如果需求模糊、存在歧义或缺失关键信息，必须主动向用户提问，切勿自行猜测。**
2. **检索**：相关类型、目录、方法、已有扩展/工具（**优先复用**）。**当确定不了解决方案、不知道如何解决，或者多次尝试仍无法解决问题时，必须主动使用搜索工具（如 Web 搜索、官方文档搜索等）从各个渠道获取最新信息。**
...
PeiKeSmart 支持 `net45` 到 `net10`，常用条件符号：`NETFRAMEWORK`、`NETSTANDARD2_0`、`NETCOREAPP`、`NET5_0_OR_GREATER`、`NET6_0_OR_GREATER`、`NET8_0_OR_GREATER`。
...
- 简体中文回复
- **需求理解度达到 95% 以上再开始编码，不清楚必须主动提问**
- **遇到未知、不确定或多次尝试失败时，必须主动通过搜索工具获取最新信息，停止无效尝试**
- 输出前检索现有实现，**禁止重复造轮子**
- 先列方案再实现
- 标记不确定上下文为"需查看文件"
